import { colorToRgb, rgbToHexColor, px } from '@/duxapp/utils'
...
export const styleConfig = {
  ...
  transformOrigin: {
    text: '变换原点',
    transform: val => {
      return `${(typeof val.x === 'number' ? px(val.x) : val.x) ?? 'center'} ${(typeof val.y === 'number' ? px(val.y) : val.y) ?? 'center'} ${val.z ? px(val.z) : 0 || 0}`
    }
  },
  transform: {
    text: '变换',
    transform: val => {
      const sizekey = ['translateX', 'translateY', 'perspective']
      return Object.keys(val).map(key => {
        if (sizekey.includes(key) && typeof val[key] === 'number') {
          return `${key}(${px(val[key])})`
        }
        return `${key}(${val[key]})`
      }).join(' ')
    }
...
const styled = {
   ...
   * @param {boolean} css 将样式转换为css输出
   * @param {boolean} platform 保留平台差异 px函数将直接以字符串的形式输出
   */
  styleTransform(style, transformClass, css, platform) {
    ...
    for (const key in style) {
      ...
      const styleItemConfig = styleConfig[key]

      if (styleItemConfig?.transform) {
        style[key] = styleItemConfig.transform(style[key])
      }
